...
| copilot  | sandbox:copilot  | OAuth   | `github.com`, `api.github.com`, `*.githubcopilot.com`, `copilot-proxy.githubusercontent.com` (+ attribution/telemetry) |
...
per-plan subdomains. See `lib/agents.sh` for the authoritative list.
...
`secrets`, `mcps`, and `services` are covered in their own sections below
("Secret store", "MCP & service dependencies").
...
**New here?** `docs/tutorials/mcp-dependency.md` is a hands-on walkthrough that
deploys a real MCP server (the official reference "everything" server) into a
session, has the agent call one of its tools, and shows you how to confirm the
sandbox actually boxed it in.
